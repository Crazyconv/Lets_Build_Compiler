...
public class StmtCodeGenerator extends Visitor<Void> {
	...
	public Void visitWhileStmt(WhileStmt nd) {
		 ...
		 */
		NopStmt loopNop = j.newNopStmt();
		NopStmt breakNop = j.newNopStmt();
		Value cond = ExprCodeGenerator.generate(nd.getExpr(), fcg);
		
		units.add(loopNop);
		units.add(j.newIfStmt(j.newEqExpr(cond, IntConstant.v(0)), breakNop));
		nd.getBody().accept(this);
		units.add(j.newGotoStmt(loopNop));
		units.add(breakNop);
		breakTargets.put(nd, breakNop);
		return null;
	}
}
